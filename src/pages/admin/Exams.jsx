...
export default function Exams() {
  ...
  // Sort exams by newest first (using createdAt, then opensAt as fallback)
  const exams = [...(data?.exams || [])].sort((a, b) => {
    const dateA = new Date(a.createdAt || a.opensAt || 0);
    const dateB = new Date(b.createdAt || b.opensAt || 0);
    ...
    <div className="space-y-8">
      ...
      {exams.length > 0 ? (
        <>
          ...
          {/* Pagination Controls */}
          {totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-8 p-4 bg-white/50 backdrop-blur-sm rounded-xl border border-gray-200">
              <p className="text-sm text-gray-600">
                Showing <span className="font-semibold">{startIndex + 1}</span>{" "}
                to{" "}
                <span className="font-semibold">
                  {Math.min(endIndex, exams.length)}
                </span>{" "}
                of <span className="font-semibold">{exams.length}</span> exams
              </p>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1 px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
                  <ChevronLeft size={18} />
                  Previous
                </button>
